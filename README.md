...
Your personal **Czech coach** inside your AI agent — short production-first drills, an honest
...
The coach explains in your native language and moves toward Czech as your level rises
(comprehensible input, i+1), and drills what Czech actually turns on — the seven cases, animacy, verbal aspect, the ř sound, vowel length, second-position clitics, and spoken/literary diglossia.

## One plugin, three hosts

The coaching skill (`skills/czech-coach/SKILL.md`) and `.mcp.json` are shared; each host reads its
...
- **Claude Code / Desktop** — via the RunDrill languages marketplace:
  ...
  /plugin install rundrill-czech@rundrill-lang
  ```
  Then run `/czech-coach`.
- **OpenAI Codex** — add the RunDrill languages catalog, then install `rundrill-czech`:
  ```
  codex plugin marketplace add rundrill/rundrill-lang
  ```
- **Google Antigravity** — drop this folder into `~/.gemini/config/plugins/rundrill-czech/` (global)
  or `<workspace>/.agents/plugins/rundrill-czech/` (workspace-scoped).
...
Required attribution when sharing: *“RunDrill Czech coach — © RunDrill, CC BY-NC-ND 4.0,
https://rundrill.com”.*
